...
export function DataTable<TData, TValue>({ columns, data, kelompokId }: DataTableProps<TData, TValue>) {
  ...
  return (
    <div>
      <div className="flex items-center justify-between py-4">
        <Input
          placeholder="Filter nama personel..."
          value={(table.getColumn('nama')?.getFilterValue() as string) ?? ''}
          onChange={(event) => table.getColumn('nama')?.setFilterValue(event.target.value)}
          className="max-w-sm"
        />
        <Link href={route('kelompok.personel.create', [kelompokId])}>
          <Button>
            <Plus />
            Create
          </Button>
